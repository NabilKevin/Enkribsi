...
const Edit = ({setShowPopup, children, setIsNotfound}) => {
  ...
  const handleSuccessGet = e => {
    ...
    for(const dat in d) {
      ...
      if(dat.includes('time')) {
        const time = d[dat].split(':')
        const date = new Date()
        d[dat] = new Date(date.setHours(time[0], time[1], time[2])) 
      }
    }
  ...
  const {data, singleExecute} = useMultipleFetch({fetchs: [HrService.editSchedule, HrService.getSchedule, HrService.getOffices], setLoading, 
    errorCallbackMap: {
      editSchedule: handleError,
      getSchedule: handleError,
      getOffices: handleError
    },
    successCallbackMap: {
      editSchedule: handleSuccess,
      getSchedule: handleSuccessGet
    }
  })
