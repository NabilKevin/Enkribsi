...
const Create = ({setShowPopup, children}) => {
  ...
  const {data, singleExecute} = useMultipleFetch({fetchs: [AdminService.createUser, AdminService.getBods], setLoading, 
    errorCallbackMap: {
      createUser: handleError,
      getBods: handleError,
    },
    successCallbackMap: {
      createUser: handleSuccess
    }
  })
  ...
  const handleSubmit = e => {
    e.preventDefault();
    singleExecute('createUser', formData)
  }
    ...
    <Container>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          ...
          <input type="password" className={`form-control ${error?.errors?.password ? 'is-invalid' : ''}`} id="passwordInput" name='password' onChange={handleChange} value={formData.password} required />
          {
