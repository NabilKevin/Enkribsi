...
const Schedule = ({children, setShowPopup}) => {
...
const {data, singleExecute} = useMultipleFetch({fetchs: [BodService.getSchedules, BodService.denySchedules, BodService.approveSchedules], setLoading,
    errorCallbackMap: {
      getSchedules: handleError,
      denySchedules: handleError,
      approveSchedules: handleError
    },
    successCallbackMap: {
      denySchedules: handleSuccess,
      approveSchedules: handleSuccess
    }
  })
  ...
  return (
    <Container>
      ...
      <div className="d-flex mb-3 align-items-center header-text-schedule">
            ...
            <tbody>
              {
                data.getSchedules.data.map((d, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <ModalBoxButton className="btn btn-dark ms-1 btn-action-schedule mt-1" callback={() => setModalContent({
                            title: 'Detail kantor',
                            content: 
                            (
                              <div className="px-2">
                          ...
                          </ModalBoxButton>
                      {
                        d.status === 'pending' &&
                        <>
                          <ModalBoxButton className="btn btn-success ms-1 btn-action-schedule mt-1" callback={() => setModalContent({
                            title: 'Konfirmasi setuju!',
                            content: <span>Yakin ingin setujui pengumuman ini? ({d.judul})</span>,
                            callback: () =>  singleExecute('approveSchedules', d.id)
                          })}>
                          ...
                          <ModalBoxButton className="btn btn-danger ms-1 mt-1 btn-action-schedule" callback={() => setModalContent({
                            title: 'Konfirmasi tolak!',
                            content: <span>Yakin ingin tolak pengumuman ini? ({d.judul})</span>,
                            callback: () =>  singleExecute('denySchedules', d.id)
                          })}>
