...
const Statistics = ({children, setShowPopup}) => {
  ...
  const { data: employees, execute: setEmployees } = useMultipleFetch({fetchs: [BodService.getEmployees], setLoading,
    errorCallbackMap: {
  ...
  return (
    <>
      ...
      <a href="/bod/statistic" className="w-100 btn btn-outline-dark z-0 mb-3" type="button" id="button-solo">Lihat semua statistik</a>
      { 
            ...
            <tbody>
              {
                employees.getEmployees.data?.map((employee, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <a href={`/bod/statistic/${employee?.username}`} className="btn btn-outline-dark">Stats</a>
                    </td>
                  </tr>
                ))
              }
