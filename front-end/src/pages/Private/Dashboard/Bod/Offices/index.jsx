...
const {data, singleExecute} = useMultipleFetch({fetchs: [BodService.getOffices, BodService.denyOffices, BodService.approveOffices], setLoading,
    errorCallbackMap: {
      getOffices: handleError,
      denyOffices: handleError,
      approveOffices: handleError
    },
    successCallbackMap: {
      denyOffices: handleSuccess,
      approveOffices: handleSuccess
    }
  })

  const handleChangePage = (page = 1) => {
    singleExecute('getOffices', page)
  }
  ...
  return (
    <Container>
      ...
      <div className="d-flex mb-3 align-items-center header-text-office">
        <h2>List pengumuman</h2>
      </div>
      <hr />
      {
        data?.getOffices?.data?.length > 0 ? 
          <>
          <Table>
            <thead>
              <tr>
                <th scope='col'>#</th>
                <th scope='col' className='text-capitalize'>Nama</th>
                <th scope='col'>Aksi</th>
              </tr>
            </thead>
            <tbody>
              {
                data.getOffices.data.map((d, i) => (
                  <tr key={i}>
                    <td>{i+1}</td>
                    <td className='scroll'>{d['nama'].length > 27 ? d['nama'].slice(0,27) + '...' : d['nama']}</td>
                    <td>
                      <ModalBoxButton className="btn btn-dark ms-1 btn-action-office mt-1" callback={() => setModalContent({
                            title: 'Detail kantor',
                            content: 
                            (
                              <div className="px-2">
                                {
                                  data?.getOffices.data && Object.keys(data?.getOffices.data[0]).map((value, i) => 
                          ...
                          </ModalBoxButton>
                      {
                        d.status === 'pending' &&
                        <>
                          <ModalBoxButton className="btn btn-success ms-1 btn-action-office mt-1" callback={() => setModalContent({
                            title: 'Konfirmasi setuju!',
                            content: <span>Yakin ingin setujui pengumuman ini? ({d.judul})</span>,
                            callback: () =>  singleExecute('approveOffices', d.id)
                          })}>
                          ...
                          <ModalBoxButton className="btn btn-danger ms-1 mt-1 btn-action-office" callback={() => setModalContent({
                            ...
                            callback: () =>  singleExecute('denyOffices', d.id)
                          })}>
          ...
          <PaginateButton datas={data.getOffices.links} handleChangePage={handleChangePage} />
          </>
          :
