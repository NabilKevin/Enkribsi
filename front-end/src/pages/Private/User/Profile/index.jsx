...
const Profile = ({setShowPopup, children, setShowNotificationButton}) => {
  ...
  return (
    <Container addClass='d-flex flex-column gap-4 mt-5'>
      {children}
      ...
      <Card>
        <div className="card-body">
          ...
          {
            data?.getAuth?.role !== 'user' &&
            <div className="mb-3">
              <a href={`/${data.getAuth.role}`} className="btn btn-danger w-100">Pergi ke dashboard {data.getAuth.role.toUpperCase()}</a>
            </div>
          }
          <div className="mb-3">
            <button className="btn btn-danger w-100" onClick={() => singleExecute('logout')}>Logout</button>
          </div>
        </div>
      </Card>
    </Container>
