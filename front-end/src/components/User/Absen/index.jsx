...
export const Form = ({handleCheckLocation, offices, grant, handleChange, formData}) => {
  return (
    <>
      ...
      <form className="mt-5" onSubmit={handleCheckLocation}>
        ...
        <select className="form-select mb-3" id="work_type" name="work_type" required disabled={!grant?.location || !grant?.webcam} value={formData.work_type} onChange={handleChange}>
          <option value="wfo">WFO (Work From Office)</option>
          <option value="wfh">WFH (Work From Home)</option>
          <option value="wfa">WFA (Work From Anywhere)</option>
        </select>
        {
          formData.work_type !== 'wfh' &&
          <>
